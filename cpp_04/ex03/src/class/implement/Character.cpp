...
AMateria *Character::getEquipment(int idx)
{
    ...
    {
        std::cout << "Character: "
                  << "Nothing found to get at index " << idx << std::endl;
        return (0);
    }
    std::cout << " warning: "
              << "You need to free this memory in the caller." << std::endl;
...
void Character::equip(AMateria *m)
{
    ...
    {
        std::cout << "Character: "
                  << "no more Materia cannot be assigned." << std::endl;
        return;
    }
    this->equipments_[i] = m;
    this->free_responsible_[i] = false;
    std::cout << "Character: "
              << "Materia assigned in index: " << i << "." << std::endl;
    return;
}
...
void Character::unequip(int idx)
{
    ...
    {
        std::cout << "Character: " << this->name_
                  << " failed to unequip(because of index out of range) in index:  " << idx << std::endl;
    }
    else if (!(this->equipments_)[idx])
    {
        std::cout << "Character: " << this->name_ << " failed to unequip(because of null address) in index: " << idx
                  << std::endl;
    }
    else
    {
        AMateria *ptr = (this->equipments_)[idx];
        std::cout << "Character: " << this->name_ << " unequipped " << ptr->getType() << " in index: " << idx
                  << std::endl;
        if (this->free_responsible_[idx])
        {
            std::cout << "warning: "
                      << "If the address is not stored in the caller via "
                         "'getEquipment', it will cause a memory leak."
                      << std::endl;
        }
...
void Character::use(int idx, ICharacter &target)
{
    ...
    {
        std::cout << "Character: "
                  << "Nothing found to use at index " << idx << std::endl;
        return;
    }
