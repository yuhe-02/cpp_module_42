...
void Character::unequip(int idx) {
  if (idx < 0 || idx >= MAX_EQUIP) {
    std::cout << "Character: " << this->name_
              << " failed to unequip(because of index out of range) in index:  "
              << idx << std::endl;
  } else if (!(this->equipments_)[idx]) {
    std::cout << "Character: " << this->name_
              << " failed to unequip(because of null address) in index: " << idx
              << std::endl;
  } else {
    AMateria *ptr = (this->equipments_)[idx];
    std::cout << "Character: " << this->name_ << " unequipped "
              << ptr->getType() << " in index: " << idx << std::endl;
