...
Sed::Sed(std::string filename, std::string before, std::string after)
    : m_filename(filename), m_before(before), m_after(after) {
  ...
  if (!this->_openFile(this->m_fileWriter, this->m_outFilename,
                       std::ios::out)) {
...
bool Sed::_openFile(std::fstream &file, std::string fileName,
                    std::ios_base::openmode mode) {
  file.open(fileName.c_str(), mode);
  return (file.is_open());
}

std::string Sed::_createReplaceWords(const std::string &content,
                                     size_t readCount) {
  int pos;
