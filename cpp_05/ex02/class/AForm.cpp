...
AForm::AForm(int signGrade, int execGrade)
    : name_("anonymous"), isAssigned_(false), signGrade_(signGrade), execGrade_(execGrade)
{
    std::cout << "AForm " << this->name_ << ": "
...
AForm::AForm(std::string name, int signGrade, int execGrade)
    : name_(name), isAssigned_(false), signGrade_(signGrade), execGrade_(execGrade)
{
    std::cout << "AForm " << this->name_ << ": "
              << "constructor with name, sign grade and exec grade called" << std::endl;
    if (this->signGrade_ > 150 || this->execGrade_ > 150)
        throw(AForm::GradeTooLowException());
    else if (this->signGrade_ < 1 || this->execGrade_ < 1)
        throw(AForm::GradeTooHighException());
...
AForm::AForm(const AForm &copy)
    : name_(copy.getName()), isAssigned_(false), signGrade_(copy.getSignGrade()), execGrade_(copy.getExecGrade())
{
    std::cout << "AForm " << this->name_ << ": "
