...
ClapTrap::ClapTrap()
    : name_(DF_NAME), hit_points_(DF_HP), energy_points_(DF_EP),
      attack_damage_(DF_AD) {
  std::cout << "ClapTrap Default Constructor called" << std::endl;
}
ClapTrap::ClapTrap(const std::string name)
    : name_(name), hit_points_(DF_HP), energy_points_(DF_EP),
      attack_damage_(DF_AD) {
  std::cout << "ClapTrap Constructor with " << this->name_ << " argument called"
            << std::endl;
}
ClapTrap::~ClapTrap() {
...
void ClapTrap::beRepaired(unsigned int amount) {
  if (this->energy_points_ <= 0) {
    std::cout << "ClapTrap " << this->name_
              << " fails to be repaired. Wait for die!" << std::endl;
    return;
  }
