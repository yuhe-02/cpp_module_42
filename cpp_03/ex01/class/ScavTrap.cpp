...
ScavTrap::ScavTrap(const std::string name) : ClapTrap(name)
{
    ...
    this->attack_damage_ = DF2_AD;
    this->is_gurdian_ = DF2_GD;
    std::cout << "ScavTrap Constructor with " << this->name_ << " argument called " << std::endl;
}
...
ScavTrap::ScavTrap(const ScavTrap &other) : ClapTrap(other), is_gurdian_(other.is_gurdian_)
{
...
void ScavTrap::attack(const std::string &target)
{
    ...
    std::cout << "ScavTrap " << this->name_ << " attacks " << target << ", causing " << this->attack_damage_
